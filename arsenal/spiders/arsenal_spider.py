...
class ArsenalSpiderSpider(scrapy.Spider):
    ...
    start_urls = ('http://voice.hupu.com/o/arsenal', )
    close_down = False
    ...
    def parse(self, response):
        ...
        for card in card_list:
            ...
            yield item
            print "yield item"
            yield scrapy.Request(item['full_text_url'],
        ...
        next_page = response.xpath(
            "//body/div[@class='hp-wrap']/div[@class='voice-main']/div[@class='voice-paging']/a/@href")
        print response.urljoin(next_page[0].extract())

    def parse_full_content(self, response):
        artical = response.xpath(
            "//body/div[@class='hp-wrap']/div[@class='voice-main']")
        artical_title = artical.xpath(
            "div[@class='artical-title']/h1/text()").extract()
        artical_src = artical.xpath(
            "div[@class='artical-title']/div[@class='artical-info']/span/span[@class='comeFrom']/a/text()").extract(
            )
        artical_date = artical.xpath(
            "div[@class='artical-title']/div[@class='artical-info']/span/a/span/text()").extract(
            )
        artical_content = artical.xpath("div[@class='artical-content']")
        artical_content_read = artical_content.xpath(
            "div[@class='artical-content-read']")
        artical_content_video = artical_content.xpath(
            "div[@class='artical-content-video']")

        if len(artical_content_read) > 0:
            print "read item"
            artical_important_pic = artical_content_read.xpath(
                "div[@class='artical-importantPic']/img/@src").extract()
            artical_main_content = artical_content_read.xpath(
                "div[@class='artical-main-content']//p").extract()
            artical_editor = artical_content_read.xpath(
                "div[@class='artical-main-content']/span/text()").extract()
            item = ArsenalArticalItem()
            item['artical_type'] = "read"
        elif len(artical_content_video) > 0:
            print "video item"
            artical_important_pic = artical_content_video.xpath(
                "div[@class='artical-video-play']/a/img/@src").extract()
            artical_main_content = artical_content_video.xpath(
                "div[@class='artical-main-content']//p").extract()
            artical_editor = artical_content_video.xpath(
                "div[@class='artical-main-content']/span/text()").extract()
            artical_video_play = artical_content_video.xpath(
                "div[@class='artical-video-play']/a/@href").extract()
            item = ArsenalArticalVideoItem()
            item['artical_type'] = "video"
            item['artical_video_play'] = self.convert_content(
                artical_video_play)
        ...
        item['artical_important_pic'] = self.convert_content(
            artical_important_pic)
        item['artical_main_content'] = self.convert_main_content(
